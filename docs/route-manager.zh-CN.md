...
普通权限状态不会启动任何隧道。只有高权限实例会创建 TUN 设备、修改路由/DNS 并启动 `gost` 子进程。

管理器不会按名称查找或终止 `gost`，因此不会影响用户在终端、服务管理器或其他程序中启动的实例。每条隧道只管理自己创建并持有句柄的进程树：Linux/macOS 使用独立进程组，Windows 使用带 `KILL_ON_JOB_CLOSE` 的 Job Object。停止时先请求进程组正常退出，超时才强制回收该任务的完整进程树；多条隧道会并行清理。若托盘退出无法确认清理成功，主程序会保持运行并提示错误，避免静默遗留后台服务。
