...
- **Porty Stealth Port Forwarding** - Carries TCP port forwarding over HTTPS/WebSocket with AEAD authentication, smux session caching, multi-port mixed-role sessions, optional P2P direct mode, dynamic `bindpath` WS bridge for Mihomo/iOS, and standalone `portyd`/`portyc` binaries. In SSH-exit mode, Gust owns SSH authentication; host-key verification is explicit opt-in, not a default or automatic pin. `portyc` connects only to the relay over WS/WSS and to the selected SSH server. Loopback targets and subsequent proxy hops still pass through SSH; no raw-TCP or exec fallback is used.
